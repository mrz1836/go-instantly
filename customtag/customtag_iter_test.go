...
func (s *CustomTagTestSuite) TestListIterWalksEveryPage() {
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		case "":
			_, _ = fmt.Fprint(w, tagPage([]string{"t1", "t2"}, "cursor-2"))
		case "cursor-2":
			_, _ = fmt.Fprint(w, tagPage([]string{"t3", "t4"}, "cursor-3"))
		default:
			_, _ = fmt.Fprint(w, tagPage([]string{"t5"}, ""))
		}
	})
...
// requests.
func (s *CustomTagTestSuite) TestListIterStopsOnBreak() {
	var requests atomic.Int64

	s.Router.Get(listPath, func(w http.ResponseWriter, _ *http.Request) {
		requests.Add(1)
		_, _ = fmt.Fprint(w, tagPage([]string{"t1", "t2", "t3"}, "cursor-2"))
	})
...
// nil tag and ends the iteration.
func (s *CustomTagTestSuite) TestListIterStopsOnError() {
	var requests atomic.Int64
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		if req.URL.Query().Get("starting_after") == "" {
			_, _ = fmt.Fprint(w, tagPage([]string{"t1"}, "cursor-2"))
			return
		}
	...
	s.Nil(errTag, "an error is yielded with no tag")
...
func (s *CustomTagTestSuite) TestListIterStopsOnCancellation() {
	var requests atomic.Int64
...
func (s *CustomTagTestSuite) TestListIterEmptyFirstPage() {
	var requests atomic.Int64
...
func (s *CustomTagTestSuite) TestListIterStopsOnEmptyPage() {
	var requests atomic.Int64
...
func (s *CustomTagTestSuite) TestListIterKeepsCallerOptions() {
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		_, _ = fmt.Fprint(w, tagPage([]string{"t2"}, ""))
	})

	callerOpts := []customtag.ListOption{
		customtag.WithSearch("vip"),
		customtag.WithStartingAfter("caller-cursor"),
	}
	...
	s.Equal([]string{"vip", "vip"}, searches, "the caller's filters must survive every page")

	s.Require().Len(callerOpts, 2)
}

// tagPage renders one page of a list response containing the given tag
// identifiers, followed by the cursor of the next page when there is one.
func tagPage(ids []string, nextCursor string) string {
	items := make([]string, 0, len(ids))
	for _, id := range ids {
		items = append(items, fmt.Sprintf(
			`{"id":%q,"label":"Tag","organization_id":"org-1",`+
