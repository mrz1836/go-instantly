...
func (s *BlockListTestSuite) TestListIterWalksEveryPage() {
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		case "":
			_, _ = fmt.Fprint(w, entryPage([]string{"b1", "b2"}, "cursor-2"))
		case "cursor-2":
			_, _ = fmt.Fprint(w, entryPage([]string{"b3", "b4"}, "cursor-3"))
		default:
			_, _ = fmt.Fprint(w, entryPage([]string{"b5"}, ""))
		}
	})
	...
	s.Equal([]string{"b1", "b2", "b3", "b4", "b5"}, seen)
...
// requests.
func (s *BlockListTestSuite) TestListIterStopsOnBreak() {
	var requests atomic.Int64

	s.Router.Get(listPath, func(w http.ResponseWriter, _ *http.Request) {
		requests.Add(1)
		_, _ = fmt.Fprint(w, entryPage([]string{"b1", "b2", "b3"}, "cursor-2"))
	})
...
// nil entry and ends the iteration.
func (s *BlockListTestSuite) TestListIterStopsOnError() {
	var requests atomic.Int64
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		if req.URL.Query().Get("starting_after") == "" {
			_, _ = fmt.Fprint(w, entryPage([]string{"b1"}, "cursor-2"))
			return
		}
...
func (s *BlockListTestSuite) TestListIterStopsOnCancellation() {
	var requests atomic.Int64
...
func (s *BlockListTestSuite) TestListIterEmptyFirstPage() {
	var requests atomic.Int64
...
func (s *BlockListTestSuite) TestListIterStopsOnEmptyPage() {
	var requests atomic.Int64
...
func (s *BlockListTestSuite) TestListIterKeepsCallerOptions() {
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		domains = append(domains, req.URL.Query().Get("domains_only"))
		...
		_, _ = fmt.Fprint(w, entryPage([]string{"b2"}, ""))
	})

	callerOpts := []blocklist.ListOption{
		blocklist.WithDomainsOnly(true),
		blocklist.WithStartingAfter("caller-cursor"),
	}
	...
	s.Equal([]string{"true", "true"}, domains, "the caller's filters must survive every page")

	s.Require().Len(callerOpts, 2)
}

// entryPage renders one page of a list response containing the given entry
// identifiers, followed by the cursor of the next page when there is one.
func entryPage(ids []string, nextCursor string) string {
	items := make([]string, 0, len(ids))
	for _, id := range ids {
		items = append(items, fmt.Sprintf(
			`{"id":%q,"bl_value":"spam.example.com","is_domain":true,"organization_id":"org-1",`+
