...
func (s *APIKeyTestSuite) TestListIterWalksEveryPage() {
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		case "":
			_, _ = fmt.Fprint(w, keyPage([]string{"k1", "k2"}, "cursor-2"))
		case "cursor-2":
			_, _ = fmt.Fprint(w, keyPage([]string{"k3", "k4"}, "cursor-3"))
		default:
			_, _ = fmt.Fprint(w, keyPage([]string{"k5"}, ""))
		}
	})
	...
	s.Equal([]string{"k1", "k2", "k3", "k4", "k5"}, seen)
...
// requests.
func (s *APIKeyTestSuite) TestListIterStopsOnBreak() {
	var requests atomic.Int64

	s.Router.Get(listPath, func(w http.ResponseWriter, _ *http.Request) {
		requests.Add(1)
		_, _ = fmt.Fprint(w, keyPage([]string{"k1", "k2", "k3"}, "cursor-2"))
	})
...
// nil key and ends the iteration.
func (s *APIKeyTestSuite) TestListIterStopsOnError() {
	var requests atomic.Int64
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		if req.URL.Query().Get("starting_after") == "" {
			_, _ = fmt.Fprint(w, keyPage([]string{"k1"}, "cursor-2"))
			return
		}
...
func (s *APIKeyTestSuite) TestListIterStopsOnCancellation() {
	var requests atomic.Int64
...
func (s *APIKeyTestSuite) TestListIterEmptyFirstPage() {
	var requests atomic.Int64
...
func (s *APIKeyTestSuite) TestListIterStopsOnEmptyPage() {
	var requests atomic.Int64
...
func (s *APIKeyTestSuite) TestListIterKeepsCallerOptions() {
	...
	s.Router.Get(listPath, func(w http.ResponseWriter, req *http.Request) {
		...
		_, _ = fmt.Fprint(w, keyPage([]string{"k2"}, ""))
	})

	callerOpts := []apikey.ListOption{
		apikey.WithLimit(25),
		apikey.WithStartingAfter("caller-cursor"),
	}
	...
	s.Equal([]string{"25", "25"}, limits, "the caller's filters must survive every page")

	s.Require().Len(callerOpts, 2)
}

// keyPage renders one page of a list response containing the given API key
// identifiers, followed by the cursor of the next page when there is one.
func keyPage(ids []string, nextCursor string) string {
	items := make([]string, 0, len(ids))
	for _, id := range ids {
		items = append(items, fmt.Sprintf(
			`{"id":%q,"name":"CI deploy key","scopes":["campaigns:read"],"key":"secret",`+
